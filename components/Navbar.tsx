...
// Bu bileşen menüdeki linklerin efektli görünmesini sağlar
const MenuLink = ({ href, label, onClick }: { href: string, label: string, onClick: (e: React.MouseEvent) => void }) => (
  <Link 
    href={href} 
    onClick={onClick} 
    className="group flex items-center text-3xl md:text-4xl font-serif text-black py-2"
  >
      {/* Soldan çıkan çizgi efekti */}
      <span className="w-0 h-[1px] bg-black mr-0 opacity-0 transition-all duration-500 ease-out group-hover:w-8 group-hover:mr-4 group-hover:opacity-100"></span>
      {/* Yazı kayma ve italik olma efekti */}
      <span className="transition-all duration-500 ease-out group-hover:italic group-hover:translate-x-2 group-hover:text-stone-600">
          {label}
      </span>
  </Link>
);
           ...
           {isMounted && (
                ...
                <SheetContent side="left" className="w-full md:w-[400px] bg-[#FDFBF7] border-r border-stone-100 p-8 md:p-10 z-[60] transition-all duration-500 ease-in-out">
                    ...
                    <div className="flex flex-col gap-4 md:gap-6">
                        {/* MenuLink bileşeni artık tanımlı olduğu için hata vermeyecek */}
                        <MenuLink href="/" label="Ana Sayfa" onClick={handleHomeClick} />
                        <MenuLink href="/services" label="Hizmetler" onClick={() => setIsOpen(false)} />
                        {/* <MenuLink href="/about" label="Hikayemiz" onClick={() => setIsOpen(false)} /> */}
                        <MenuLink href="/contact" label="İletişim" onClick={() => setIsOpen(false)} />
                        ...
                        <div className="mt-8 md:mt-16 pt-8 md:pt-10 border-t border-stone-200">
                            ...
                            <a href="tel:+905368994374" className="text-xl font-serif text-black mb-2 block">
                             +90 (536) 899 43 74
                            </a>
                            <a href="mailto:gavgavpetkuafor@gmail.com" className="text-lg font-serif text-stone-600 block">
        ...
        <div className="flex-1 flex justify-center">
            ...
            >
                ...
                <div className="flex items-end gap-[2px] mb-[2px]">
                    <PawPrint 
                        className="w-3 h-3 text-black fill-black/10 -rotate-12 mb-[1px] opacity-60 transition-transform duration-300 group-hover/logo:-translate-x-1" 
                        strokeWidth={1.5}
                    />
                     <PawPrint 
                        className="w-4 h-4 text-black fill-black/10 rotate-12 transition-transform duration-300 group-hover/logo:rotate-0" 
                        strokeWidth={1.5}
                    />
                </div>
